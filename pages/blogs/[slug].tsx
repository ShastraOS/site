...
export default function BlogPost({ frontmatter, code, relatedPosts }: BlogPost) {
  ...
  } on the @shastraos blogs.&url=https://shastraos.co/blogs/${frontmatter.slug}
		`;

  return (
    <>
      <Titles title={`${frontmatter.title} — ShastraOS`} poster={frontmatter.poster} />
...
export async function getStaticProps(context) {
  const { frontmatter, code } = await getMdxBySlug('blogs', context.params.slug);

  const relatedPosts = frontmatter.relatedIds
    ? await Promise.all(
        frontmatter.relatedIds.map(async (id) => {
          const { frontmatter } = await getMdxBySlug('blogs', id);
          return frontmatter;
        })
      )
    : null;
